...
class MyQMC5883LComponent : public PollingComponent, public i2c::I2CDevice {
 public:
  ...
  void setup() override {
    ...
    delay(100);
    // 2. Set/Reset Period — обязательно, без этого мусор/нули
    write_reg_(0x0A, 0x01);
    delay(50);
    // 3. CR1: Continuous mode, ±8G, 50Hz, OSR=512
    write_reg_(0x0B, 0x1D);
    delay(50);
    // 4. CR2: без прерываний
  ...
  void update() override {
    ...
    if (this->write(&reg, 1) != i2c::ERROR_OK) {
      ESP_LOGW("qmc5883l", "I2C write failed");
      return;
    }
    uint8_t data[6];
    if (this->read(data, 6) != i2c::ERROR_OK) {
      ESP_LOGW("qmc5883l", "I2C read failed");
      return;
    }
    ...
    // ±8G: 3000 LSB/Gauss, 1 Gauss = 100 µT → 1 LSB = 0.03333 µT
